...
class DeepSeekChatMonitor {
    ...
    /**
     * Выполняет клик через debugger API (доверенный клик) по селектору
    ...
    async _performContinueClick(element) {
        ...
        const selector = this._getUniqueSelector(element);

        if (!selector) {
            this._log('continue', '⚠️ Не удалось получить селектор для элемента');
            // Пробуем старый метод с координатами как fallback
            const rect = element.getBoundingClientRect();
            const x = rect.left + rect.width / 2;
            const y = rect.top + rect.height / 2;
            this._log('continue', `🖱️ Fallback: клик по координатам (${Math.round(x)}, ${Math.round(y)})`);
            await this._attemptTrustedClick(x, y);
            return;
        }

        this._log('continue', `🖱️ Попытка клика по элементу с селектором: ${selector}`);

        // Выполняем доверенный клик через background по селектору
        await this._attemptTrustedClickBySelector(selector);
    }

    /**
     * Получение уникального CSS селектора для элемента
    ...
    _getUniqueSelector(element) {
        ...
        // Используем путь по классам + позиции
        const path = [];
        let current = element;
        ...
        while (current && current !== document.body) {
            ...
            if (current.className && typeof current.className === 'string') {
                const classes = current.className.split(' ').filter(c => c && !c.match(/^_/));
                if (classes.length > 0) {
        ...
        // Проверяем, что селектор уникальный
        try {
            if (document.querySelectorAll(fullSelector).length === 1) {
                return fullSelector;
            }
        } catch (e) {
            // Селектор может быть некорректным
        }

        // Если не уникальный - ищем родителя с id
        let parent = element.parentElement;
        while (parent && parent !== document.body) {
            if (parent.id) {
                const parentSelector = `#${parent.id} ${element.tagName.toLowerCase()}`;
                try {
        ...
        // Фолбек - используем XPath
        return this._getXPath(element);
    }

    /**
     * Получение XPath для элемента
     * @param {Element} element - DOM элемент
     * @returns {string|null}
     */
    _getXPath(element) {
    ...
    /**
     * Попытка доверенного клика через background по селектору
     * @param {string} selector - CSS селектор элемента
     * @returns {Promise<boolean>}
     */
    async _attemptTrustedClickBySelector(selector) {
        if (typeof chrome === 'undefined' || !chrome.runtime) {
            this._log('continue', '⚠️ chrome.runtime не доступен');
            return false;
        }

        return new Promise((resolve) => {
            const timeout = setTimeout(() => {
                this._log('continue', '⏰ Таймаут ожидания ответа от background');
                resolve(false);
            }, 5000);
            ...
            try {
                chrome.runtime.sendMessage({
                    ...
                    if (chrome.runtime.lastError) {
                        this._log('continue', `⚠️ Ошибка: ${chrome.runtime.lastError.message}`);
                        resolve(false);
                        return;
                    }

                    if (response && response.success) {
                        this._log('continue', '✅ Доверенный клик выполнен (isTrusted: true)');
                        this._lastResponseText = '';
                        this._lastResponseHtml = '';
                        this._lastMutationTime = Date.now();
                        this.resetMutationIdleTimer();
                        ...
                        this._log('continue', `⚠️ Клик не удался: ${response?.error || 'unknown error'}`);
                        resolve(false);
                    }
                });
            } catch (e) {
                clearTimeout(timeout);
                this._log('continue', `⚠️ Исключение: ${e.message}`);
                resolve(false);
            }
        });
    }
    ...
    /**
     * Попытка доверенного клика через background (старый метод - по координатам)
     * @param {number} x - X координата
     * @param {number} y - Y координата
     * @returns {Promise<boolean>}
     */
    async _attemptTrustedClick(x, y) {
        if (typeof chrome === 'undefined' || !chrome.runtime) {
            this._log('continue', '⚠️ chrome.runtime не доступен');
            return false;
        }
        ...
        return new Promise((resolve) => {
            ...
            try {
                chrome.runtime.sendMessage({
                    ...
                    if (chrome.runtime.lastError) {
                        this._log('continue', `⚠️ Ошибка при отправке: ${chrome.runtime.lastError.message}`);
                        resolve(false);
                        return;
                    }
                    ...
                    if (response && response.success) {
                        ...
                        this._log('continue', `⚠️ Доверенный клик не удался: ${response?.error || 'unknown error'}`);
                        resolve(false);
                    }
                });
            } catch (e) {
                clearTimeout(timeout);
                this._log('continue', `⚠️ Исключение при отправке: ${e.message}`);
                resolve(false);
            }
        });
    }
    ...
    handleContinueButton(continueButton) {
        ...
        if (this.isAutoClickEnabled()) {
            this._log('continue', '🖱️ Автоматическое нажатие кнопки "Continue" (разрешено флагом)');
    ...
    _printFinalTaskStats(task, validation) {
        ...
        const hasTags = /<\/?[a-z][\s\S]*>/i.test(task.assistantResponseHtml || '');
