...
function isExcluded(name, type, fullPath = '') {
    ...
    const respectExcludePatterns = config?.scanOptions?.respectExcludePatterns !== false;
    ...
    if (type === 'directory') {
        ...
        if (fullPath && excludedDirs.some(dir => fullPath.includes(`/${dir}/`) || fullPath.endsWith(`/${dir}`))) {
            return true;
        }
    }
...
export function loadConfig(configPath = './config.json') {
    try {
        ...
        if (!fs.existsSync(configFile)) {
            console.warn(`⚠️  Configuration file ${configPath} not found. Using default configuration.`);
        ...
        const excludeExcludedDirs = config.scanOptions?.excludeExcludedDirs !== false;
        if (excludeExcludedDirs) {
            directoryConfigs = (config.directories || []).filter(dir => !dir.excluded);
            const excludedCount = (config.directories || []).length - directoryConfigs.length;
            if (excludedCount > 0) {
                const lang = getCurrentLanguage();
...
function getDefaultConfig() {
    return {
        directories: [
            { id: "10", name: "Directory/10", description: "Пользовательская документация", description_en: "User documentation", required: false, excluded: false },
            { id: "11", name: "Directory/11", description: "Пользовательские функции", description_en: "User functions", required: false, excluded: false },
            { id: "12", name: "Directory/12", description: "Пользовательские файлы", description_en: "User files", required: false, excluded: false }
        ],
        excludePatterns: {
            directories: ["node_modules", ".git", "dist", "build", "coverage", ".nyc_output", "__pycache__", ".cache", "logs"],
            files: ["README.md", "LICENSE.md", "package-lock.json", "yarn.lock", "pnpm-lock.yaml", ".DS_Store", "Thumbs.db", "*.log", "*.tmp", "*.swp"]
        },
        scanOptions: {
            recursive: true,
...
function isSupportedFile(extension, fileName) {
    const supportedExtensions = config?.supportedExtensions || getDefaultConfig().supportedExtensions;
    const specialFiles = config?.specialFiles || getDefaultConfig().specialFiles;

    return supportedExtensions.includes(extension.toLowerCase()) || specialFiles.includes(fileName);
}
...
function scanDirectory(dirPath, virtualPath = '', currentDepth = 0) {
    ...
    try {
        ...
        for (const item of items) {
            ...
            const itemVirtualPath = virtualPath ? `${virtualPath}/${item.name}` : `/${item.name}`;

            // Проверка на исключение по паттерну
            if (isExcluded(item.name, item.isDirectory() ? 'directory' : 'file', fullPath)) {
                const lang = getCurrentLanguage();
                if (item.isDirectory()) {
                    result.children[item.name] = {
                        ...
                        "status": lang === 'english' ? "[excluded by pattern]" : "[исключено по паттерну]",
            ...
            if (item.isDirectory()) {
                if (config?.scanOptions?.recursive !== false) {
                    result.children[item.name] = scanDirectory(fullPath, itemVirtualPath, currentDepth + 1);
                } else {
                    result.children[item.name] = {
                        "@type": "Directory",
                        "type": "directory",
                ...
                if (isSupportedFile(ext, item.name)) {
                    result.children[item.name] = processFile(fullPath, itemVirtualPath, ext);
                } else {
                    result.children[item.name] = {
                        "type": "file",
                        "path": itemVirtualPath,
...
function getDirectoryDescription(dirName) {
    const lang = getCurrentLanguage();
    const descriptions = lang === 'english' ? config?.descriptions_en : config?.descriptions;
...
export function saveScanReport(stats, outputPath, append = false) {
    try {
        ...
        if (reportDir !== '.' && !fs.existsSync(reportDir)) {
            ...
            const lang = getCurrentLanguage();
            console.log(`📁 ${lang === 'english' ? 'Created directory' : 'Создана директория'}: ${reportDir}`);
        }

        const shouldAppend = reportConfig.append !== undefined ? reportConfig.append : append;
        ...
        console.log(`${t('reportSize')}: ${(fs.statSync(actualPath).size / 1024).toFixed(2)} KB`);
...
export function collectScanStats(fileSystem) {
    ...
    if (fileSystem["."] && fileSystem["."].children) {
        ...
        for (const [name, dir] of Object.entries(children)) {
            ...
            const isExcluded = dir.status === "[excluded]" ||
            ...
            const isMissing = dir.status === "[missing]" ||
                dir.status === "[отсутствует]";

            // Проверяем на ошибку
            const isError = dir.status === "[error]" ||
                dir.status === "[ошибка]";
            ...
            if (isExcluded) {
                ...
                stats.errors++;
            } else if (dir.type === "directory" && (!dir.status || dir.status === "[ok]" || dir.status === "[целый]" || dir.status === undefined)) {
                stats.scannedCount++;
                ...
                if (dir.children) {
                    filesCount = Object.values(dir.children).filter(
                        child => child.type === "file" && child.status !== "[skipped]" && child.status !== "[пропущен]" && child.status !== "[excluded by pattern]" && child.status !== "[исключено по паттерну]"
                    ).length;
                }
...
export function createFileSystemJSON(configPath = './config.json') {
    ...
    const fileSystem = {
        ".": {
            ...
            "metadata": config?.includeMetadata !== false ? {
                ...
                "excludedDirectories": (config?.directories || []).length - directories.length
            } : undefined
        }
    };
    ...
    for (const dirConfig of directories) {
        ...
        try {
            if (fs.existsSync(realPath)) {
                fileSystem["."].children[dirConfig.name] = scanDirectory(realPath, virtualPath);
                const lang = getCurrentLanguage();
                console.log(lang === 'english'
                ...
                const lang = getCurrentLanguage();
                const missingStatus = lang === 'english' ? "[missing]" : "[отсутствует]";
                ...
                fileSystem["."].children[dirConfig.name] = {
                    ...
                    "description": getLocalizedDescription(dirConfig),
                    "required": isRequired,
                    "warning": !isRequired ? (lang === 'english' ? "Directory not found on disk" : "Директория не найдена на диске") : undefined,
                    "children": {}
                };
            }
        } catch (error) {
    ...
    console.log(`   ${t('totalDirectories')}: ${(config?.directories || []).length}`);
    console.log(`   ${t('excludedFromScan')}: ${excludedCount}`);
    console.log(`   ${t('successfullyScanned')}: ${directories.length - excludedCount - missingCount}`);
    ...
    if (excludedCount > 0) {
        ...
        console.log(`   ${lang === 'english' ? 'To include them, change "excluded" parameter in config.json' : 'Чтобы включить их, измените параметр "excluded" в config.json'}`);
    }
    ...
    if (missingCount > 0) {
        ...
        console.log(`   ${lang === 'english' ? 'To add missing directories, create them in Directory/ folder' : 'Чтобы добавить отсутствующие директории, создайте их в папке Directory/'}`);
    }
...
export function saveFileSystemJSON(outputPath, fileSystem, prettyPrint = true) {
    try {
        ...
        if (outputDir !== '.' && !fs.existsSync(outputDir)) {
            ...
            const lang = getCurrentLanguage();
            console.log(`📁 ${lang === 'english' ? 'Created directory' : 'Создана директория'}: ${outputDir}`);
        }
        ...
        console.log(`📏 ${lang === 'english' ? 'File size' : 'Размер файла'}: ${(fs.statSync(outputPath).size / 1024).toFixed(2)} KB`);
    } catch (error) {
