...
function isExcluded(name, type, fullPath = '') {
  ...
  if (type === 'directory') {
    ...
    if (
      fullPath &&
      excludedDirs.some(dir => fullPath.includes(`/${dir}/`) || fullPath.endsWith(`/${dir}`))
    ) {
      return true;
    }
  }
...
function isSupportedFile(extension, fileName) {
  ...
  const supportedExtensions = config?.supportedExtensions || DEFAULT_SUPPORTED_EXTENSIONS;
...
export function collectScanStats(fileSystem) {
  ...
  if (fileSystem['.'] && fileSystem['.'].children) {
    ...
    for (const [name, dir] of Object.entries(children)) {
      ...
      const isMissing = dir.status === '[missing]' || dir.status === '[отсутствует]';
